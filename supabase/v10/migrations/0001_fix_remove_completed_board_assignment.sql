-- V10-A2.1
-- Corrige remocao de associacao multiquadro ja concluida.
--
-- Regra:
-- remover uma associacao altera assignment_status para removed,
-- mas preserva completed_at/completed_by quando o status operacional
-- ja representa conclusao.
--
-- A associacao removida deixa de participar do status global porque
-- recalculate_work_item_global_status considera somente vinculos ativos.
...
begin
  ...
  if not found then
    raise exception 'Distribui????o ativa n??o encontrada.';
  end if;
  ...
  then
    raise exception 'Voc?? n??o possui permiss??o para remover esta associa????o.';
  end if;
