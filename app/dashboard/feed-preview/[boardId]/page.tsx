...
export default async function FeedBoardPage({ params }: { params: { boardId: string } }) {
  ...
  const [clientResult, itemsResult, eventsResult, assetsResult] = await Promise.all([
    ...
    supabase
      ...
      .select('id,board_id,work_item_id,position,title,cover_url,storage_path,content_type,source_file_name,content_url,caption,scheduled_date,scheduled_time,internal_notes,approval_status,workflow_status,client_feedback,approved_at,created_at,updated_at')
      .eq('board_id', boardId)
