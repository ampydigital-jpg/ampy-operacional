...
export default async function DashboardPage() {
  ...
  const hoje = dateKeyInAmpyTimezone()
  const in3 = addDateKeyDays(hoje, 3)
  const last30 = addDateKeyDays(hoje, -30)
  ...
  const [
    ...
    supabase.from('work_items').select('*',{count:'exact',head:true}).eq('status','blocked'),
  ...
  const statusData = Object.entries(statusCont).map(([k,v])=>({key:k,name:statusLabels[k]||k,value:v}))
  ...
  return (
    <DashboardCharts
      metrics={{activeClients:clientesAtivos??0, openDemands:demandasAbertas??0, lateDemands:demandasAtrasadas??0, blockedDemands:bloqueadas??0}}
