  ...
  return (
    <div className="page-wrap">
      ...
      <div style={{display:'flex', height:'calc(100vh - 56px)', overflow:'hidden'}}>
        {/* LISTA — sem dados de pagamento */}
        <div style={{flex:1, overflowY:'auto', padding:'16px'}}>
          <table style={{width:'100%', borderCollapse:'collapse'}}>
            <thead>
              <tr>
                {['Cliente','Segmento','Cidade','Serviços','Início','Término','Status',''].map(h => (
            ...
            <tbody>
              {filtered.map((c: any) => {
                ...
                return (
                  <tr key={c.id} onClick={() => { setSelected(c); setEditing(false) }} style={{cursor:'pointer', background:selected?.id===c.id?'var(--s2)':''}}
                    onMouseEnter={e => { if(selected?.id!==c.id) e.currentTarget.style.background='var(--s1)' }}
                    ...
                    <td style={{padding:'10px 12px',borderBottom:'0.5px solid #141414'}}>
                      <div style={{display:'flex',alignItems:'center',gap:'9px'}}>
                        ...
                        <div>
                          ...
                          {c.main_contact_phone&&<div style={{fontSize:'9px',color:'var(--t4)'}}>{c.main_contact_phone}</div>}
                        </div>
                      </div>
                    </td>
                    ...
                    <td style={{padding:'10px 12px',borderBottom:'0.5px solid #141414',fontSize:'11px',color:'var(--t2)'}}>{formatDate(c.inicio_contrato)}</td>
                    <td style={{padding:'10px 12px',borderBottom:'0.5px solid #141414'}}>
                      <span style={{fontSize:'10px',fontWeight:600,color:expiring?'var(--warn)':'var(--t2)'}}>{formatDate(c.fim_contrato)}{expiring&&' ⚠'}</span>
                    </td>
                    <td style={{padding:'10px 12px',borderBottom:'0.5px solid #141414'}}>
                      <span className="badge bok"><span className="dot" style={{background:'var(--ok)'}}/> Ativo</span>
                    </td>
                    <td style={{padding:'10px 12px',borderBottom:'0.5px solid #141414'}}>
        ...
        {/* PAINEL LATERAL — com todos os dados incluindo pagamento */}
        {selected && !editing && (
          <div className="panel">
            ...
            <div className="panel-body">
              ...
              <div className="prow"><span className="pkey">Vencimento</span><span className="pval">Todo dia {selected.dia_vencimento||'—'}</span></div>
              <div className="prow"><span className="pkey">Pagamento</span><span className="pval">{selected.metodo_pagamento||'—'}</span></div>
              <div className="prow"><span className="pkey">Notas fiscais</span><span className="pval">{selected.notas_fiscais||'—'}</span></div>
              <div className="prow"><span className="pkey">Duração</span><span className="pval">{selected.tempo_contrato||'—'}</span></div>
              ...
              {selected.drive_folder_url
                ? <a href={selected.drive_folder_url} target="_blank" rel="noopener noreferrer" className="lrow"><i className="ti ti-brand-google-drive" style={{color:'var(--ok)',fontSize:'13px'}}/><span>Pasta no Drive</span><i className="ti ti-external-link" style={{color:'var(--t4)',fontSize:'12px'}}/></a>
                : <div style={{fontSize:'11px',color:'var(--t4)',padding:'6px 0'}}>Nenhum link cadastrado</div>}
              ...
              <div className="psec">Feed Preview</div>
              <a href={`/dashboard/clientes/feed-preview?clientId=${selected.id}`} className="lrow">
                <i className="ti ti-grid-dots" style={{color:'var(--purple)',fontSize:'13px'}}/>
                <span>Abrir Feed Preview</span>
                <i className="ti ti-arrow-right" style={{color:'var(--t4)',fontSize:'12px'}}/>
              </a>
            ...
            <div className="panel-foot">
              ...
              <button className="pbpri" onClick={() => setEditing(true)}>Editar</button>
            </div>
          </div>
        )}
        ...
        {selected && editing && (
          <div className="panel" style={{width:'400px',minWidth:'400px'}}>
            <div className="panel-head">
              <div style={{flex:1}}><div style={{fontSize:'13px',fontWeight:600,color:'var(--w)'}}>Editar — {selected.name}</div></div>
            ...
            <form onSubmit={handleUpdate} style={{display:'flex',flexDirection:'column',height:'100%'}}>
              <div className="panel-body">
                ...
                <div className="frow">
                  <div className="fg"><label className="fl">Email</label><input className="fi" name="main_contact_email" type="email" defaultValue={selected.main_contact_email||''}/></div>
              ...
              <div className="panel-foot">
                ...
                <button type="submit" className="pbpri" disabled={loading}>{loading?'Salvando...':'Salvar'}</button>
              </div>
            </form>
          </div>
        )}
