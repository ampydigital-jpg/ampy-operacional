...
export default async function AvisosPage() {
  ...
  const [
    ...
    supabase.from('feed_board_items').select('id,board_id,title,position,approval_status,client_feedback,scheduled_date,scheduled_time,updated_at,created_at').order('updated_at', { ascending: false }).limit(2000),
  ...
  const loadErrors = [
    ...
    boardsResult.error ? `AprovaÃ§Ãµes: ${boardsResult.error.message}` : null,
    itemsResult.error ? `Itens de aprovaÃ§Ã£o: ${itemsResult.error.message}` : null,
    eventsResult.error ? `HistÃ³rico de aprovaÃ§Ãµes: ${eventsResult.error.message}` : null,
