...
export default async function PublicApprovalPage({ params }: { params: { token: string } }) {
  ...
  const [clientResult, itemsResult, eventsResult] = await Promise.all([
    ...
    supabase
      ...
      .select('id,board_id,position,title,cover_url,content_type,source_file_name,content_url,caption,scheduled_date,scheduled_time,approval_status,workflow_status,client_feedback,approved_at,created_at,updated_at')
